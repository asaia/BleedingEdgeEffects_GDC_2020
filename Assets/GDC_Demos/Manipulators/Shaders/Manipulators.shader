...
Shader "GDC Demo/Manipulators"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma shader_feature _VISUALIZE_FALLOFF
            #pragma shader_feature _DEBUG_NORMALS
            #pragma shader_feature _CORRECT_NORMALS
            ...
            Varyings LitPassVertex(Attributes input)
            {
                ...
                //Apply Manipulator
                float3 manipulatedPositionWS = ApplyManipulators(vertexInput.positionWS);

                //Recaculate normals and tangents
                #ifdef _CORRECT_NORMALS
                float3 manipulatedTangentWS = ApplyManipulators(vertexInput.positionWS + vertexNormalInput.tangentWS * 0.01);
                float3 manipulatedBitangentWS = ApplyManipulators(vertexInput.positionWS + vertexNormalInput.bitangentWS * 0.01);
                output.tangentWS = normalize(manipulatedTangentWS - manipulatedPositionWS);
                output.bitangentWS = normalize(manipulatedBitangentWS - manipulatedPositionWS);
                output.normalWS = normalize(cross(output.bitangentWS, output.tangentWS));
                #endif

                vertexInput.positionWS = manipulatedPositionWS;
            ...
            half4 LitPassFragment(Varyings input) : SV_Target
            {
                ...
                for (int i = 0; i < _NumManipulators; i++)
                {
                    ...
                    //falloff += SphereMask(positionWS, anchorPosition.xyz, radius, hardness);
                    falloff += ComputeFalloff(positionWS, anchorPosition.xyz, radius * hardness, radius);
                }

                color = lerp(color, float3(1, 0, 0), falloff);
                #endif
